...
from agents.a2c.models import ActorCritic
...
class A2C:
    def __init__(self, args, device, load, path1, path2):
        ...
        self.feature_history = args.feature_history  # length of CGM / Ins
        self.n_features = args.n_features  # 2 -> CGM, Ins
        ...
        self.avg_rew = 0
        self.avg_rew_lr = args.reward_lr
    ...
    def compute_average_targets(self):
        orig_device = self.v_pred.device
        assert orig_device == self.reward.device == self.first_flag.device
        vpred, reward, first = (x.cpu() for x in (self.v_pred, self.reward, self.first_flag))
        first = first.to(dtype=torch.float32)
        ...
        adv = torch.zeros(nenv, nstep, dtype=torch.float32)
        lastgaelam = 0
        sum_delta = torch.zeros(1)
        sum_rew = torch.zeros(1)
        for t in reversed(range(nstep)):
            notlast = 1.0 - first[:, t + 1]
            nextvalue = vpred[:, t + 1]
            # notlast: whether next timestep is from the same episode
            delta = (reward[:, t] - self.avg_rew) + notlast * self.gamma * nextvalue - vpred[:, t]
            sum_delta += torch.sum(delta)
            adv[:, t] = lastgaelam = delta + notlast * self.gamma * self.lambda_ * lastgaelam
            #experiments
            # sum_rew = torch.sum(reward[:, t])
            #sum_delta += torch.sum(adv)

        vtarg = vpred[:, :-1] + adv
        # update the average reward

        self.avg_rew = self.avg_rew + (self.avg_rew_lr * sum_delta) / (self.n_step * self.n_training_workers)
        #self.avg_rew = (sum_rew) / (self.n_step * self.n_training_workers)

        print('The updated avg reward : {}'.format(self.avg_rew))
        return adv.to(device=orig_device), vtarg.to(device=orig_device)
    ...
    def prepare_rollout_buffer(self):
        ...
        self.rollout_buffer = dict(s_hist=s_hist, act=act, logp=logp, ret=v_targ, adv=adv, len=buffer_len)
    ...
    def update(self, rollout):
        ...
        if self.return_type == 'average':
            ...
            #self.adv, self.v_targ = self.compute_average_targets()
