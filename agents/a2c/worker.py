...
from agents.a2c.core import Memory, StateSpace, composite_reward
...
class Worker:
    def __init__(self, args, mode, patients, env_ids, seed, worker_id, device):
        ...
        # todo: should be moved to wrappers.
    ...
    def init_env(self):
        ...
        self.cur_state = self.state_space.update(cgm=self.init_state.CGM, ins=0)
    ...
    def calibration_process(self):
        ...
        for t in range(0, self.calibration):  # open-loop simulation for calibration period.
            ...
            cur_cgm = state.CGM
            self.cur_state = self.state_space.update(cgm=state.CGM, ins=self.std_basal)
    ...
    def rollout(self, policy):
        ...
        for n_steps in range(0, rollout_steps):
            ...
            # todo: refactor
            #selected_action = policy_step['action'][0] if self.worker_mode == 'training' else policy_step['mu'][0]
            if self.worker_mode == 'training':
                selected_action = policy_step['action'][0]
            else:
                #selected_action = policy_step['mu'][0]
                selected_action = policy_step['action'][0]
            ...
            if self.worker_mode == 'training':   # store -> rollout for training
                self.memory.store(self.cur_state, policy_step['action'][0], reward, policy_step['state_value'],
                                  policy_step['log_prob'], self.counter)
            self.cur_state = self.state_space.update(cgm=state.CGM, ins=pump_action)  # update -> state.

            self.episode_history[self.counter] = [self.episode, self.counter, state.CGM, info['meal'] * info['sample_time'],
                                                  pump_action, reward, rl_action, policy_step['mu'][0], policy_step['mu'][0],
