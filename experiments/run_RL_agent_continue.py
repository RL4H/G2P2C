...
def setup_folders(args):
    ...
    os.makedirs(LOG_DIR + '/training/plots', exist_ok=True)
    os.makedirs(LOG_DIR + '/testing/data', exist_ok=True)
    os.makedirs(LOG_DIR + '/testing/plots', exist_ok=True)
...
def set_agent_parameters(args):
    ...
    if args.agent == 'ppo':
        ...
        #! 아래와 같이 수정하세요:
        weights = MAIN_PATH + '/results/' + args.folder_id + '/checkpoints/'
        agent = G2P2C(args, device, True, weights+'episode_5_Actor.pth', weights+'episode_5_Critic.pth')
        
        # MAX_INTERACTIONS 값을 직접 수정 (예: 절반으로 줄임)
        agent.MAX_INTERACTIONS = 2000 if args.debug == 1 else 40000
        agent.LR_DECAY_INTERACTIONS = 1000 if args.debug == 1 else 30000
