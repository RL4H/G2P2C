...
class ExperimentVisualise:
    ...
    def get_value_function(self, horizon):
        ...
        state_val = get_concat_recent(self.MAIN_PATH, self.training_seeds, 'training/data/logs_worker_', 'state_val',
                                      horizon)
    ...
    def get_test_episode(self, tester, episode):
        if self.version == 1.0:
            df = pd.read_csv(
                self.MAIN_PATH + '/testing/data/logs_test_worker_' + str(self.testing_seeds[tester]) + '.csv')
...
def plot_episode(experiment, tester, episode):
    ...
    for t in range(0, len(df)):
        if df.iloc[t]['meal']:
            ...
            ax.annotate('Carbohydrates: ' + str(df.iloc[t]['meal']) + 'g', (df.iloc[t]['time'], off_set),
                        color=meal_color)  # df.iloc[t]['cgm']
...
def plot_episode_new(experiment, tester, episode):
    ...
    for t in range(0, len(df)):
        if df.iloc[t]['meal']:
            off_set = (max_cgm - 125) if x else (max_cgm - 75)
            ax.annotate('Carbohydrates: ' + str(df.iloc[t]['meal']) + 'g', (df.iloc[t]['time'], off_set),
                        color=meal_color, size=12)  # df.iloc[t]['cgm']
            ax.plot((df.iloc[t]['time'], df.iloc[t]['time']), (df.iloc[t]['cgm'], off_set), color=meal_color)
